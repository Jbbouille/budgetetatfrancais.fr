...
(() => {
  ...
  const parentDe = (c) => (c.length > 4 ? c.slice(0, c.length - 2) : null);
  ...
  function lireURL() {
    ...
    if (valide(aOuvrir)) {
      ...
      ouvrirRepli(aOuvrir);
      stack.push({ kind: 'fn', code: aOuvrir, label: shortLabel(DATA.apu[aOuvrir].label) });
    }
